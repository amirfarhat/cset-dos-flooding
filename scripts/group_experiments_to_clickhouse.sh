...
usage() {
  cat <<EOM
  Usage:
    $(basename $0) -e exp_name_inputs -d db_name -n no_bootstrap
...
if [[ -z "$exp_name_inputs" ]] || [[ -z "$db_name" ]] || [[ -z "$no_bootstrap" ]]; then
  usage;
  exit 1
fi
...
python3 $SCRIPT_HOME/send_experiment_group_to_clickhouse.py -i $exp_dirs -d $db_name